...
class ApplierDomain:
    ...
    def _apply_desktop_widget_addon(self, postfix: str) -> None:
        ...
        # 1. Define Paths
        # The Rust widget expects theme.css in ~/.config/meowterialyou-widgets/mediawidget/theme.css
        # We can eventually make this shared for all widgets
        config_dir = os.path.join(home, ".config/meowterialyou-widgets/media_widget")
        os.makedirs(config_dir, exist_ok=True)
        theme_file = os.path.join(config_dir, "theme.css")
        ...
        # Based on what styles.rs looks for
...
"""
        ...
        try:
            ...
            log.error(f"Failed to write widget theme: {e}")
            return

        # 4. Reload Widgets
        # Since the widget doesn't watch for file changes yet, we restart it
        # Restarting the manager handles all children
        try:
            # Check if running first to avoid error spam
            subprocess.run(
                ["pkill", "-f", "meowterialyou-widget-manager"], capture_output=True
            )
            subprocess.run(["pkill", "-f", "media_widget"], capture_output=True)

            # Start manager in background if configured to autostart
            # Ideally we check if it WAS running, but for now we assume enabled implies running
            ...
            if os.path.exists(manager_bin):
                ...
                log.info("Restarted meowterialyou-widget-manager")
        except Exception as e:
            log.warning(f"Failed to restart widget manager: {e}")

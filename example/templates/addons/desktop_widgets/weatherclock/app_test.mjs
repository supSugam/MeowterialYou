  ...
  "weatherclock/src/ui/styles.ts"() {
    ...
    applyStyles = (config) => {
      ...
      css.load_from_data(`
        ...
        .clock {
             font-size: ${config.typography.clock_size};
             font-weight: 800;
             color: @widget_text;
             margin-bottom: ${s(-8)}px;
        }
        .date {
             font-size: ${config.typography.date_size};
             font-weight: 600;
             color: @widget_primary;
             margin-bottom: ${s(8)}px;
        }
        .weather-icon {
             /* color: @widget_text; GtkImage doesn't always perform coloring this way unless symbolic */
        }
        .weather-temp {
             font-size: ${s(24)}px;
             font-weight: 700;
        ...
        .separator {
            min-height: ${s(24)}px;
            min-width: 1px;
            background-color: alpha(@widget_text, 0.2);
            margin: 0 ${s(12)}px;
        }
    `);
...
var init_weather = __esm({
  "weatherclock/src/services/weather.ts"() {
    ...
    initWeather = () => {
      ...
      if (world) _location = world;
      _info = new GWeather.Info({ application_id: "org.gnome.Weather" });
      _info.connect("updated", () => {
      });
    };
    getWeather = () => {
      if (!_info) initWeather();
      const temp = _info.get_value_temp(GWeather.TemperatureUnit.DEFAULT);
      const tStr = _info.get_temp();
      const summary = _info.get_weather_summary();
      const iconName = _info.get_icon_name();
      return { temp: tStr, summary, iconName };
    };
  }
});
...
var win, timeLabel, dateLabel, greetingLabel, weatherIcon, weatherTemp, weatherDesc, statLabels, buildUI, updateClock, updateStats, updateWeather, startWindow;
var init_main = __esm({
  "weatherclock/src/ui/main.ts"() {
    "use strict";
    init_styles();
    ...
    buildUI = (config) => {
      ...
      const leftBox = new Gtk2.Box({ orientation: Gtk2.Orientation.VERTICAL, valign: Gtk2.Align.CENTER });
      timeLabel = new Gtk2.Label({ label: "00:00", halign: Gtk2.Align.START });
      timeLabel.get_style_context().add_class("clock");
      dateLabel = new Gtk2.Label({ label: "...", halign: Gtk2.Align.START });
      dateLabel.get_style_context().add_class("date");
      greetingLabel = new Gtk2.Label({ label: "", halign: Gtk2.Align.START });
      greetingLabel.get_style_context().add_class("stat-label");
      leftBox.pack_start(timeLabel, false, false, 0);
      leftBox.pack_start(dateLabel, false, false, 0);
      leftBox.pack_start(greetingLabel, false, false, 0);
      const statsGrid = new Gtk2.Grid({ column_spacing: 12, row_spacing: 4, valign: Gtk2.Align.CENTER });
      statsGrid.set_margin_start(24);
      const addStat = (label, key, row, col) => {
        const l = new Gtk2.Label({ label, halign: Gtk2.Align.START });
        l.get_style_context().add_class("stat-label");
        const v = new Gtk2.Label({ label: "-", halign: Gtk2.Align.START });
        v.get_style_context().add_class("stat-value");
        statLabels[key] = v;
        statsGrid.attach(l, col * 2, row, 1, 1);
        statsGrid.attach(v, col * 2 + 1, row, 1, 1);
      };
      addStat("UP:", "uptime", 0, 0);
      addStat("CPU:", "load", 1, 0);
      addStat("MEM:", "mem", 0, 1);
      addStat("TMP:", "temp", 1, 1);
      const weatherBox = new Gtk2.Box({ orientation: Gtk2.Orientation.VERTICAL, valign: Gtk2.Align.CENTER, halign: Gtk2.Align.END });
      weatherBox.set_hexpand(true);
      weatherIcon = new Gtk2.Image({ icon_name: "weather-clear-symbolic", pixel_size: 32 });
      weatherIcon.get_style_context().add_class("weather-icon");
      weatherTemp = new Gtk2.Label({ label: "--\xB0C", halign: Gtk2.Align.END });
      weatherTemp.get_style_context().add_class("weather-temp");
      weatherDesc = new Gtk2.Label({ label: "...", halign: Gtk2.Align.END });
      weatherDesc.get_style_context().add_class("weather-desc");
      const weatherRow = new Gtk2.Box({ orientation: Gtk2.Orientation.HORIZONTAL, spacing: 8, halign: Gtk2.Align.END });
      weatherRow.pack_start(weatherTemp, false, false, 0);
      weatherRow.pack_start(weatherIcon, false, false, 0);
      weatherBox.pack_start(weatherRow, false, false, 0);
      weatherBox.pack_start(weatherDesc, false, false, 0);
      mainBox.pack_start(leftBox, false, false, 0);
      mainBox.pack_start(statsGrid, false, false, 0);
      mainBox.pack_start(weatherBox, true, true, 0);
      win.add(mainBox);
      win.set_keep_below(true);
      win.stick();
      mainBox.add_events(Gdk2.EventMask.BUTTON_PRESS_MASK);
    ...
    updateClock = (config) => {
      ...
      let emoji = config.emoji.morning;
      if (hour >= 12 && hour < 17) emoji = config.emoji.afternoon;
      else if (hour >= 17 && hour < 22) emoji = config.emoji.evening;
      else if (hour >= 22 || hour < 5) emoji = config.emoji.night;
      greetingLabel.label = `${emoji} Good ${hour < 12 ? "Morning" : hour < 18 ? "Afternoon" : "Evening"}`;
    };
    updateStats = () => {
      const s = getSystemStats();
      if (statLabels["uptime"]) statLabels["uptime"].label = s.uptime;
      if (statLabels["load"]) statLabels["load"].label = s.load;
      if (statLabels["mem"]) statLabels["mem"].label = s.mem;
      if (statLabels["temp"]) statLabels["temp"].label = s.temp;
    };
    updateWeather = () => {
      const w = getWeather();
      if (w.temp) weatherTemp.label = w.temp;
      if (w.summary) weatherDesc.label = w.summary;
      if (w.iconName) weatherIcon.icon_name = w.iconName;
    };
    startWindow = (config) => {
      initWeather();
      buildUI(config);
      ...
      updateStats();
      updateWeather();
      GLib3.timeout_add_seconds(GLib3.PRIORITY_DEFAULT, 1, () => {
        updateClock(config);
        return true;
      });
      GLib3.timeout_add_seconds(GLib3.PRIORITY_DEFAULT, 5, () => {
        updateStats();
        return true;
      });
      GLib3.timeout_add_seconds(GLib3.PRIORITY_DEFAULT, 1800, () => {
        updateWeather();
        return true;
      });

...
// weatherclock/src/utils.ts
import Gio from "gi://Gio?version=2.0";
var log, readFileAsync;
...
// weatherclock/src/config.ts
import GLib from "gi://GLib?version=2.0";
import Gio2 from "gi://Gio?version=2.0";
var defaultConfig, SCRIPT_DIR, CONFIG_PATH, THEME_CSS_PATH, loadConfig;
var init_config = __esm({
  "weatherclock/src/config.ts"() {
    ...
    defaultConfig = {
      ...
      emoji: { morning: "\u2615", afternoon: "\u2600\uFE0F", evening: "\u{1F319}", night: "\u{1F634}" },
      typography: { clock_size: "48px", date_size: "16px" }
    };
    SCRIPT_DIR = GLib.path_get_dirname(imports.system.programInvocationName);
    CONFIG_PATH = `${SCRIPT_DIR}/config.yaml`;
    THEME_CSS_PATH = `${SCRIPT_DIR}/theme.css`;
    loadConfig = () => {
      let config = { ...defaultConfig };
      try {
        const file = Gio2.File.new_for_path(CONFIG_PATH);
        const [success, contents] = file.load_contents(null);
        if (success) {
          const decoder = new TextDecoder("utf-8");
          const parsed = load(decoder.decode(contents));
          if (parsed) {
...
// weatherclock/src/ui/styles.ts
import Gtk from "gi://Gtk?version=3.0";
import Gdk from "gi://Gdk?version=3.0";
import GLib2 from "gi://GLib?version=2.0";
import Gio3 from "gi://Gio?version=2.0";
var Layout, themeContent, bgStyle, calculatedOpacity, loadStyles, updateLayoutFromEnv, applyStyles;
var init_styles = __esm({
  "weatherclock/src/ui/styles.ts"() {
    ...
    themeContent = "";
    bgStyle = "smart_transparency";
    calculatedOpacity = 60;
    loadStyles = () => {
      try {
        const tFile = Gio3.File.new_for_path(THEME_CSS_PATH);
    ...
    updateLayoutFromEnv = () => {
      const envX = GLib2.getenv("WIDGET_MARGIN_X");
      if (envX) Layout.marginX = parseInt(envX, 10);
      const envY = GLib2.getenv("WIDGET_MARGIN_Y");
      if (envY) Layout.marginY = parseInt(envY, 10);
      const envStack = GLib2.getenv("WIDGET_STACK_OFFSET_Y");
      if (envStack) Layout.stackOffsetY = parseInt(envStack, 10);
      const envPos = GLib2.getenv("WIDGET_POSITION_MODE");
      if (envPos) Layout.positionMode = envPos;
      const envWidth = GLib2.getenv("WIDGET_WIDTH_OVERRIDE");
    ...
    applyStyles = (config) => {
      ...
      if (bgStyle === "smart_transparency") {
        bgOpacity = calculatedOpacity / 100;
      } else {
        bgOpacity = config.background ? config.background.opacity / 100 : 0.6;
      }
      const scale = config.layout.scale_factor || 1;
...
// weatherclock/src/services/system.ts
var getSystemStats;
  ...
  "weatherclock/src/services/system.ts"() {
    ...
    getSystemStats = () => {
      ...
      try {
        ...
        if (t) tempVal = parseInt(t) / 1e3;
      } catch (e) {
      }
      temp = `${Math.round(tempVal)}\xB0C`;
      return { uptime, load: load2, mem, temp };
    };
  }
});

// weatherclock/src/services/weather.ts
import GWeather from "gi://GWeather?version=4.0";
import Gio4 from "gi://Gio?version=2.0";
function getGnomeWeatherLocation() {
  try {
    const settings = new Gio4.Settings({ schema_id: "org.gnome.Weather" });
...
function getWeatherIconChar(iconName) {
  if (!iconName) return "\u{F0599}";
  const lower = iconName.toLowerCase();
  if (lower.includes("clear") && lower.includes("night")) return "\uF186";
  if (lower.includes("clear") || lower.includes("sunny")) return "\uF185";
  if (lower.includes("few-clouds") || lower.includes("partly")) return "\u{F0595}";
  if (lower.includes("overcast") || lower.includes("cloud")) return "\u{F0590}";
  if (lower.includes("fog") || lower.includes("mist")) return "\u{F0591}";
  if (lower.includes("shower")) return "\u{F0596}";
  if (lower.includes("rain")) return "\u{F0597}";
  if (lower.includes("snow")) return "\u{F0598}";
  if (lower.includes("storm") || lower.includes("thunder")) return "\u{F0593}";
  return "\uF185";
}
var _info, _location, initWeather, getWeather;
  ...
  "weatherclock/src/services/weather.ts"() {
    ...
    initWeather = (config) => {
      ...
      if (sysLoc) {
        ...
        _location = GWeather.Location.new_detached("Pokhara", "VNPK", 28.2096 * (Math.PI / 180), 83.9856 * (Math.PI / 180));
      }
    ...
    getWeather = (config) => {
      ...
      const unitSymbol = config.weather.unit === "F" ? "\xB0F" : "\xB0";
...
function createVSpacer() {
  const s = new Gtk2.Box({ orientation: Gtk2.Orientation.VERTICAL });
  s.set_vexpand(true);
  return s;
}
function createHSpacer() {
  const s = new Gtk2.Box({ orientation: Gtk2.Orientation.HORIZONTAL });
  s.set_hexpand(true);
  return s;
}
var win, dateLabel, timeLabel, ampmLabel, wIcon, wTemp, wDesc, wCity, windLabel, humidLabel, sysCpu, sysRam, sysNet, sysTemp, buildUI, updateClock, updateStats, updateWeather, startWindow;
var init_main = __esm({
  "weatherclock/src/ui/main.ts"() {
    "use strict";
    init_styles();
    ...
    buildUI = (config) => {
      ...
      const wrapper = new Gtk2.Box({ orientation: Gtk2.Orientation.VERTICAL });
      wrapper.get_style_context().add_class("background-layer");
      const glassOverlay = new Gtk2.Box({ orientation: Gtk2.Orientation.VERTICAL });
      glassOverlay.get_style_context().add_class("glass-overlay");
      const content = new Gtk2.Box({
      ...
      const scale = config.layout.scale_factor || 1;
      const s = (v) => Math.round(v * scale);
      const dateRow = new Gtk2.Box({ orientation: Gtk2.Orientation.HORIZONTAL, spacing: s(12) });
      dateLabel = new Gtk2.Label({ label: "...", ellipsize: Pango.EllipsizeMode.END });
      dateLabel.get_style_context().add_class("date");
      if (config.layout.alignment === "right") dateRow.pack_end(dateLabel, false, false, 0);
      ...
      const timeRow = new Gtk2.Box({ orientation: Gtk2.Orientation.HORIZONTAL });
      const timeGroup = new Gtk2.Box({ orientation: Gtk2.Orientation.HORIZONTAL, spacing: s(6) });
      timeLabel = new Gtk2.Label({ label: "...", ellipsize: Pango.EllipsizeMode.END });
      timeLabel.get_style_context().add_class("time");
      ampmLabel = new Gtk2.Label({ label: "AM", valign: Gtk2.Align.END, margin_bottom: s(8) });
      ...
      if (config.layout.alignment === "right") timeRow.pack_end(timeGroup, false, false, 0);
      else timeRow.pack_start(timeGroup, false, false, 0);
      content.pack_start(timeRow, false, false, 0);
      content.pack_start(createVSpacer(), true, true, 0);
      const weatherRow = new Gtk2.Box({ orientation: Gtk2.Orientation.HORIZONTAL, spacing: 0 });
      const tempBox = new Gtk2.Box({ orientation: Gtk2.Orientation.HORIZONTAL, valign: Gtk2.Align.CENTER });
      wIcon = new Gtk2.Label({ label: "\uF185" });
      wIcon.get_style_context().add_class("weather-icon");
      wTemp = new Gtk2.Label({ label: "--" });
      ...
      const infoBox = new Gtk2.Box({ orientation: Gtk2.Orientation.VERTICAL, valign: Gtk2.Align.CENTER });
      wDesc = new Gtk2.Label({ label: "Unknown", halign: Gtk2.Align.END, ellipsize: Pango.EllipsizeMode.END });
      wDesc.get_style_context().add_class("weather-desc");
      wCity = new Gtk2.Label({ label: "Location", halign: Gtk2.Align.END, ellipsize: Pango.EllipsizeMode.END });
      ...
      const wSpring = new Gtk2.Box({ hexpand: true });
      ...
      content.pack_start(createVSpacer(), true, true, 0);
      const detailRow = new Gtk2.Box({ orientation: Gtk2.Orientation.HORIZONTAL, margin_top: s(6) });
      windLabel = new Gtk2.Label({ label: "\u{F059D} --" });
      windLabel.get_style_context().add_class("detail");
      humidLabel = new Gtk2.Label({ label: "\u{F058E} --%", margin_left: s(16) });
      ...
      content.pack_start(createVSpacer(), true, true, 0);
      const divider = new Gtk2.Box({ orientation: Gtk2.Orientation.HORIZONTAL });
      divider.get_style_context().add_class("divider");
      content.pack_start(divider, false, false, 0);
      content.pack_start(createVSpacer(), true, true, 0);
      const sysRow = new Gtk2.Box({ orientation: Gtk2.Orientation.HORIZONTAL, hexpand: true });
      const createSys = (icon, val) => {
        const b = new Gtk2.Box({ orientation: Gtk2.Orientation.HORIZONTAL, spacing: s(6) });
        const i = new Gtk2.Label({ label: icon });
        i.get_style_context().add_class("sys-icon");
        const l = new Gtk2.Label({ label: val });
        l.get_style_context().add_class("detail");
        b.pack_start(i, false, false, 0);
        b.pack_start(l, false, false, 0);
        return { box: b, label: l };
      };
      const cpu = createSys("\uF4BC", "0%");
      sysCpu = cpu.label;
      const ram = createSys("\uEFC5", "0%");
      sysRam = ram.label;
      const net = createSys("\uF019", "0 K");
      sysNet = net.label;
      const temp = createSys("\uF2C9", "0\xB0C");
      sysTemp = temp.label;
      ...
      win.stick();
      wrapper.add_events(Gdk2.EventMask.BUTTON_PRESS_MASK);
      ...
      win.connect("destroy", Gtk2.main_quit);
    };
    updateClock = (config) => {
    ...
    updateWeather = (config) => {
      ...
      if (w.wind) windLabel.label = `\u{F059D} ${w.wind}`;
      if (w.humidity) humidLabel.label = `\u{F058E} ${w.humidity}`;
    };
    startWindow = (config) => {
      initWeather(config);
      buildUI(config);
      ...
      updateStats();
      updateWeather(config);
      GLib3.timeout_add(GLib3.PRIORITY_DEFAULT, 1e3, () => {
        updateClock(config);
        return true;
      });
      GLib3.timeout_add(GLib3.PRIORITY_DEFAULT, 5e3, () => {
        updateStats();
        return true;
      });
      GLib3.timeout_add_seconds(GLib3.PRIORITY_DEFAULT, config.weather.refresh_interval_min * 60, () => {
        updateWeather(config);
        return true;
      });

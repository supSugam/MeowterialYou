function changeSvgs(arr) {
  function updateSVGs() {
    for (const s of arr) {
      ...
      svgs.forEach(svg => {
        // Narrow check to ensure we only target the correct icons
        const label = svg.closest('button')?.getAttribute('aria-label') || '';
        const isNavigation = label.includes('Previous') || label.includes('Next') || label.includes('Skip') || label.includes('Options') || label.includes('More');
    ...
    // Liked Songs Image Swap (Surgical JS fallback)
    for (const el of document.querySelectorAll('.x-entityImage-imagePlaceholder[aria-label="Liked Songs"]')) {
      if (el.style.backgroundImage !== 'url("liked_songs.png")') {
          el.style.backgroundImage = 'url("liked_songs.png")';
